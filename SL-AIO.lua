...
--end Zyra
	...
	SLS.SB:Slider("dV","Danger Value",2,1,4,1)
    ...
    DelayAction(function()
		for l,k in pairs(GetEnemyHeroes()) do
			if not self.s[k.charName] then return end
			for m,p in pairs(self.s[k.charName]) do
				...
				if not p.spellType then p.spellType = "Line" end
					if not SLS.SB.Spells[p.name] then SLS.SB.Spells:Menu(p.name,""..k.charName.." | "..(self.str[p.Slot] or "?").." - "..p.name) end
						SLS.SB.Spells[p.name]:Boolean("Dodge"..p.name, "Enable Dodge", true)
						SLS.SB.Spells[p.name]:Boolean("IsD"..p.name,"Dangerous", p.Dangerous or false)		
						SLS.SB.Spells[p.name]:Info("Empty12"..p.name, "")
						SLS.SB.Spells[p.name]:Slider("radius"..p.name,"Radius",(p.radius or 150), ((p.radius-50) or 50),((p.radius+100) or 250), 5)
						SLS.SB.Spells[p.name]:Slider("d"..p.name,"Danger",(p.danger or 1), 1, 4, 1)	
			end
		end
    end, .001)
...
function HitMe:Ti()
	if SLS.SB.uS:Value() then
		for _,i in pairs(self.object) do
			if i.o and i.spell.spellType == "Line" and GetDistance(myHero,i.o) >= 3000 then return end
			if i.p and i.spell.spellType == "Circular" and GetDistance(myHero,i.p.endPos) >= 3000 then return end
			...
			for kk,k in pairs(GetEnemyHeroes()) do
				...
				if i.o and not i.o.valid and i.spell.spellName ~= "LuxMaliceCannon" then
					self.object[_] = nil
				end
				if i.p then
					self.dT = (i.spell.delay*.001) + GetDistance(myHero,i.p.startPos) / i.spell.speed
					self.endposs = Vector(i.p.startPos)+Vector(Vector(i.p.endPos)-i.p.startPos):normalized()*(i.spell.range+i.spell.radius)
				end
				if ((not self.DoD and SLS.SB.dV:Value() <= SLS.SB.Spells[i.spell.name]["d"..i.spell.name]:Value()) or (self.DoD and SLS.SB.Spells[i.spell.name]["IsD"..i.spell.name]:Value())) then
					if (i.spell.spellType == "Line" or i.spell.spellType == "Cone" or i.spell.spellType == "Rectangle") and i.p then
						...
						if GetDistance(i.p.startPos) < i.spell.range + myHero.boundingRadius and GetDistance(self.endposs) < i.spell.range + myHero.boundingRadius then
							...
							if i.jp and GetDistance(myHero,i.jp) < i.spell.radius + myHero.boundingRadius then
								_G[ChampName]:HitMe(k,i.p,self.dT*self.fT*.001,i.spellType)
							end
						end
					elseif i.spell.spellType == "Circle" then
...
function HitMe:MinionCollision()
	for _,i in pairs(self.object) do
		if i.spell.spellType == "Line" and i.spell.collision and i.p and SLS.SB.EC:Value() then
			local vI = nil 
			local helperVec = nil
...
function HitMe:CreateObj(obj)
	if obj and obj.isSpell and obj.spellOwner.isHero and obj.spellOwner.team == MINION_ENEMY then
		for _,l in pairs(self.s[obj.spellOwner.charName]) do
			if obj.spellName:lower():find("attack") then return end
			if self.s[obj.spellOwner.charName] and SLS.SB.Spells[l.name] and SLS.SB.dV:Value() <= SLS.SB.Spells[l.name]["d"..l.name]:Value() and (l.missileName == obj.spellName or obj.spellName:lower():find(l.name:lower()) or obj.spellName:lower():find(l.spellName:lower()) or (tostring(l.extraMissileNames) and l.extraMissileNames == obj.spellName)) then
				...
				self.object[obj.spellName].mpos = nil
				self.object[obj.spellName].uDodge = nil
				self.object[obj.spellName].startTime = os.clock()
				self.object[obj.spellName].spell = l
				self.object[obj.spellName].samples = {}
...
function HitMe:Detect(unit,spellProc)
	if unit and unit.isHero and unit.team == MINION_ENEMY then
		for _,l in pairs(self.s[unit.charName]) do
			if self.s[unit.charName] and SLS.SB.Spells[l.name] and SLS.SB.dV:Value() <= SLS.SB.Spells[l.name]["d"..l.name]:Value() and (l.missileName == spellProc.name or spellProc.name:lower():find(l.name:lower()) or spellProc.name:lower():find(l.spellName:lower()) or (tostring(l.extraMissileNames) and l.extraMissileNames == spellProc.name)) then
				...
				self.object[spellProc.name].mpos = nil
...
function SLEvade:__init()
	...
	self.supportedtypes = {["Line"]={supported=true},["Circular"]={supported=true},["Cone"]={supported=false},["Rectangle"]={supported=false},["Arc"]={supported=false},["Ring"]={supported=false},["Threeway"]={supported=false},["Sevenway"]={supported=false}}
	...
	DelayAction(function()
		for l,k in pairs(GetEnemyHeroes()) do
			if not self.Spells[k.charName] then return end
			for m,p in pairs(self.Spells[k.charName]) do
				if p.name == "" and GetCastName(k,m) then p.name = GetCastName(k,m) end
				if p.spellName == "" and GetCastName(k,m) then p.spellName = GetCastName(k,m) end
				if not p.spellType then p.spellType = "Line" end
					if not EMenu.Spells[p.name] then EMenu.Spells:Menu(p.name,""..k.charName.." | "..(self.str[p.Slot] or "?").." - "..p.name) end
						...
						EMenu.Spells[p.name]:Slider("radius"..p.name,"Radius",(p.radius or 150), ((p.radius-50) or 50),((p.radius+100) or 250), 5)
						if p.Dangerous then EMenu.Spells[p.name]:Slider("hp"..p.name, "HP to Dodge", 100, 1, 100, 5)
						else EMenu.Spells[p.name]:Slider("hp"..p.name, "HP to Dodge", 85, 1, 100, 5)
						end
						...
						if p.collision then
							-- EMenu.Spells[p.name]:Boolean("hColl"..p.name, "Hero Collision", true)
							EMenu.Spells[p.name]:Boolean("mColl"..p.name, "Minion Collision", true)
							EMenu.Spells[p.name]:Boolean("wColl"..p.name, "Wall Collision", true)
						else
							-- EMenu.Spells[p.name]:Info("nohColl"..p.name, "No Hero Collision available")
							EMenu.Spells[p.name]:Info("nomColl"..p.name, "No Minion Collision available")
							EMenu.Spells[p.name]:Info("nowColl"..p.name, "No Wall Collision available")
						end	
			end
		end
				...
				-- if i.spell.spellType == "Line" then
					-- if i.o then
						...
						-- if i.samples then
							...
							-- i.spell.speed = distance/time 
						-- end
					-- else 
						-- i.spell.speed = self.Spelldatasave[_].speed
					-- end
				-- end
...
function SLEvade:Others()
	...
	for _,i in pairs(self.obj) do
		...
		if i.safe and i.spell.spellType == "Line" and i.p then
			if GetDistance(self.opos)/i.spell.speed + (i.spell.killTime or 0) + (i.spell.delay*.001) < GetDistance(i.safe)/myHero.ms then 
					i.uDodge = true 
				else
					i.uDodge = false
			end
		elseif i.safe and i.spell.spellType == "Circular" and i.p then
			if GetDistance(i.p.endPos)/i.spell.speed + (i.spell.killTime or 0) + (i.spell.delay*.001) < GetDistance(i.safe)/myHero.ms then
					i.uDodge = true 
				else
					i.uDodge = false
			end
		...
		if i.spell.spellType == "Line" and i.p then
			...
			if GetDistance(i.p.startPos) < i.spell.range + myHero.boundingRadius and GetDistance(self.endposs) < i.spell.range + myHero.boundingRadius then
				...
				if i.jp and (GetDistance(self:Position(),i.jp) < i.spell.radius + myHero.boundingRadius) or (GetDistance(myHero,i.jp) < i.spell.radius + myHero.boundingRadius) and not i.safe and i.mpos and not i.coll then
					--if GetDistance(GetOrigin(myHero) + Vector(i.p.startPos-i.p.endPos):perpendicular(),jp) >= GetDistance(GetOrigin(myHero) + Vector(i.p.startPos-i.p.endPos):perpendicular2(),jp) then
						self.asd = true
						self.patha = Vector(i.mpos)+Vector(Vector(i.mpos)-i.p.endPos):normalized():perpendicular() * ((i.spell.radius + myHero.boundingRadius)*.5+EMenu.Advanced.ew:Value())
...
function SLEvade:CreateObject(obj)
	if obj and obj.isSpell and obj.spellOwner.isHero and obj.spellOwner.team == MINION_ENEMY then
		for _,l in pairs(self.Spells[obj.spellOwner.charName]) do
			if obj.spellName:lower():find("attack") then return end
...
function SLEvade:Detection(unit,spellProc)
	if unit and unit.isHero and unit.team == MINION_ENEMY then
		for _,l in pairs(self.Spells[unit.charName]) do
			if self.Spells[unit.charName] and EMenu.Spells[l.name] and EMenu.d:Value() <= EMenu.Spells[l.name]["d"..l.name]:Value() and (l.missileName == spellProc.name or spellProc.name:lower():find(l.name:lower()) or spellProc.name:lower():find(l.spellName:lower()) or (tostring(l.extraMissileNames) and l.extraMissileNames == spellProc.name)) then
